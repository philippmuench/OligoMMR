...
wgs_data <- oligomm_ab_wgs_imputed_wide
dat <- wgs_data
data(oligomm_ab_isolates) # for annotation purpose
iso_data <- oligomm_ab_isolates
```
...
df_wide_bug <- df_wide_bug[which(df_wide_bug$observations_vancomycin >= 8 &
                                   df_wide_bug$observations_tetracyclin >= 8 &
                                   df_wide_bug$observations_control >= 8 &
                                   df_wide_bug$observations_ciprofloxacin >= 8 ),]
...
```{r}
# get the order of columns right
orders <- data.frame(
...
iso_data$snp_ids <- paste0(iso_data$chr,"-" ,iso_data$POS, "-",  iso_data$REF,"-",  iso_data$ALT)
in_iso <- ifelse(snp_ids %in% iso_data$snp_ids == TRUE, "in_isolates", "not_in_isolates")
